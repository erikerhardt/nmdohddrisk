...
date_Current          = "2022-02-01"
DD_Group              = "All"
date_today            = lubridate::today()
path_prefix_out       = "out_V17"
sw_read_data          = TRUE
sw_run_model          = TRUE
working_directory     = "D:/Dropbox/StatAcumen/consult/Industry/2023_NM_DOH_DDWaiver/data/Ydrive/Risk_Prediction_Model/Scripts"
#working_directory     = "S:/Risk_Prediction_Model/Scripts"
sw_Win_drive_subst    = FALSE # "S:" # set sw_Win_drive_subst to FALSE if working_drive is short
sw_Win_path_to_subst  = "D:/Dropbox/StatAcumen/consult/Industry/2023_NM_DOH_DDWaiver/data/Ydrive"
...
# if Windows and sw_Win_drive_subst set, then SUBST the drive to a drive letter
#   resolves some issues with very long file names
if (Sys.info()["sysname"] == "Windows" & !isFALSE(sw_Win_drive_subst)) {
  working_drive_win <-
    sw_Win_path_to_subst |>
  ...
  # substitute drive letter for working_drive path
  system(cmd_subst)
} # Win subst
...
fn_in_qmd <-
  paste0("__", fn_root, "_SOURCE_V17_20260210.qmd")
fn_out <-
...
# add quotes as a string
if (!isFALSE(sw_Win_drive_subst)) {
  sw_Win_drive_subst  <- paste0("\"", sw_Win_drive_subst, "\"")
}
...
PARAMS <-
  ...
  , PARAMS_sw_Win_drive_subst   = sw_Win_drive_subst
  , PARAMS_sw_Win_path_to_subst = sw_Win_path_to_subst
