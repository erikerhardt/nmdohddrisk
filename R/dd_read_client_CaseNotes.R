...
dd_read_client_CaseNotes <-
  ...
  names_list_CaseNotes <-
    tibble::tribble(
      ~Detail                     , ~Questionnaire                      , ~Weights
    , "CaseNotes_DD_Detail"       , "CaseNotes_DD_Questionnaire"        , "DD Waiver RPM Questions with Weights.xlsx"
  ...
  for (i_group in seq_along(names_list_Detail)) {
    ...
    dat_sheet_detail <-
      erikmisc::e_read_data_subdir_into_lists(
        fn_path                 = file.path(path_data, names_list_CaseNotes$Detail[i_group])
      , fn_detect               = "xlsx$"
    ...
    if(is.null(dat_sheet_detail)) {
      print(paste0("CaseNotes, no files: ", names_list_CaseNotes$Detail[i_group]))
      next
    }
    ...
    dat_sheet_questionnaire <-
      erikmisc::e_read_data_subdir_into_lists(
        fn_path                 = file.path(path_data, names_list_CaseNotes$Questionnaire[i_group])
      , fn_detect               = "xlsx$"
      , sw_fn_or_dat            = c("fn", "dat")[2]
      , sw_exclude_empty_dir    = c(TRUE, FALSE)[1]
      , sw_dat_add_col_path_fn  = c(TRUE, FALSE)[2]
    ...
    if(is.null(dat_sheet_detail)) {
      print(paste0("CaseNotes, no files: ", names_list_CaseNotes$Questionnaire[i_group]))
      next
    }
    ...
    dat_sheet_weights <-
      readxl::read_xlsx(
        path = file.path(path_data, names_list_CaseNotes$Weights[i_group])
      )
    ...
    dat_client_CaseNotes_all[[ i_group ]] <-
      dat_client_CaseNotes_Q |>
      dplyr::left_join(
        dat_client_CaseNotes_Weights
      ...
      dplyr::summarize(
        # Normalize by dividing by number of questions
        CaseNotes_AtRisk = sum(CaseNotes_AtRisk, na.rm = TRUE) / nrow(dat_sheet_weights)
      , .groups = "drop"
      ) |>
      dplyr::ungroup() |>
      dplyr::select(
