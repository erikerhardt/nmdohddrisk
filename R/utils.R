...
msg <- function(..., startup = FALSE) {
  if (startup) {
    if (!isTRUE(getOption("nmdohddrisk.quiet"))) {
...
#' List all packages in the nmdohddrisk
#'
#' @param include_self Include nmdohddrisk in the list?
#' @export
#' @examples
#' nmdohddrisk_packages()
nmdohddrisk_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("nmdohddrisk")$Imports
  imports <- strsplit(raw, ",")[[1]]
  parsed <- gsub("^\\s+|\\s+$", "", imports)
  names <- vapply(strsplit(parsed, "\\s+"), "[[", 1, FUN.VALUE = character(1))

  if (include_self) {
    names <- c(names, "nmdohddrisk")
  }
